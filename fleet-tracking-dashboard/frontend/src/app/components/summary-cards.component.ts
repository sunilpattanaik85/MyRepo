...
<div class="grid summary-grid" style="gap:8px;">
  <div class="card"><div>Total Vehicles</div><div style="font-size:16px;font-weight:700;">{{s?.totalVehicles || 0}}</div></div>
  <div class="card"><div>Active Vehicles</div><div style="font-size:16px;font-weight:700;">{{s?.activeVehicles || 0}}</div></div>
  <div class="card"><div>Avg Speed</div><div style="font-size:16px;font-weight:700;">{{(s?.averageSpeed || 0) | number:'1.0-1'}} km/h</div></div>
  <div class="card"><div>Total Distance Today</div><div style="font-size:16px;font-weight:700;">{{(s?.totalDistanceTodayKm || 0) | number:'1.0-1'}} km</div></div>
  <div class="card"><div>Active Corridors</div><div style="font-size:16px;font-weight:700;">{{s?.activeCorridors || 0}}</div></div>
</div>
`})
